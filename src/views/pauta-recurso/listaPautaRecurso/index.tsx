...
const ListPauta: React.FC<PautaProps> = () => {
  ...
  useEffect(() => {
    try {
      api.get("/pautaRecursoAll").then((response) => setPauta(response.data));
    } catch (err) {
  ...
  async function update({ id }: PautaProps) {
    localStorage.setItem("code-pauta-recurso", id);
    history.push(`/pautaRecurso/edit/${id}`);
  }

  async function deleteItem(params: any) {
    try {
      const id = params?.id;
      const result = await api.delete(`/pautaRecursoDelete/${id}`);
