...
const EditInscricao: React.FC<InscricaoData> = () => {
  ...
  useEffect(() => {
    try {
      api.get("/estadoAll").then((response) => setEstado(response.data));
    } catch (err) {
            ...
            <CCard>
              ...
              <CCardBody>
                <form onSubmit={handleUpdateInscricao}>
                  <CFormGroup row>
                    <CCol xs="12" md="14">
                      <CLabel htmlFor="estadoId">Estado</CLabel>
                      <CSelect
                        id="estadoId"
                        name="estadoId"
                        value={estadoId}
                        onChange={(e) =>
                          setEstadoId((e.target as HTMLInputElement).value)
                        }
                      >
                        <option value="0">
                          Por favor, selecciona um estado
                        </option>
                        {estado.map((item: any) => {
                          return (
