...
const EditPauta: React.FC<PautaData> = () => {
  ...
  const id = localStorage.getItem("code-pauta-recuperacao");
  ...
  useEffect(() => {
    async function getEstudante() {
      await api
        .get(`/pautaRecuperacao/${id}`)
        .then((response) => response.data)
        .then((result) => {
          setNotaRecuperacao(result.notaRecuperacao);
  ...
  async function handleUpdatePauta(event: FormEvent) {
    ...
    try {
      setLoading(true);
      const result = await api.patch(`/pautaRecuperacaoUpdate/${id}`, {
        notaRecuperacao,
      ...
      setLoading(false);
      Swal.fire("Pauta Recupercao!", "Notas editadas com sucesso", "success");
          ...
          <CCol xs="12">
            <CFade timeout={300} in={showElements} unmountOnExit={true}>
              <CCard>
                ...
                >
                  Edita aqui as informações da pauta de recuperação
                    ...
                    <CFormGroup row>
                      <CCol xs="12" md="14">
                        ...
                        <CInput
                          ...
                          id="mediaFinal"
                          name="mediaFinal"
                          value={parseFloat(notaRecuperacao)}
                          onChange={() => {
