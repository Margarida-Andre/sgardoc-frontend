...
const ListPauta: React.FC<PautaProps> = () => {
  ...
  useEffect(() => {
    try {
      api
        .get("/pautaRecuperacaoAll")
        .then((response) => setPauta(response.data));
    } catch (err) {
  ...
  async function update({ id }: PautaProps) {
    localStorage.setItem("code-pauta-recuperacao", id);
    history.push(`/pautaRecuperacao/edit/${id}`);
  }

  async function deleteItem(params: any) {
    try {
      const id = params?.id;
      const result = await api.delete(`/pautaRecuperacaoDelete/${id}`);
