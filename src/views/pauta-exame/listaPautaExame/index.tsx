...
const ListPauta: React.FC<PautaProps> = () => {
  ...
  useEffect(() => {
    try {
      api.get("/pautaExameAll").then((response) => setPauta(response.data));
    } catch (err) {
  ...
  async function update({ id }: PautaProps) {
    localStorage.setItem("code-pauta-exame", id);
    history.push(`/pautaExame/edit/${id}`);
  }

  async function deleteItem(params: any) {
    try {
      const id = params?.id;
      const result = await api.delete(`/pautaExameDelete/${id}`);
